...
def generar_reportes():
    ...
    try:
        ...
        progress(0.3, desc="Iniciando generación de reporte...")
        
        report_path = eda_manager.generate_ydata_report()
        
        if report_path.startswith("Error:"):
            gr.Warning(report_path)
            return None, None
            
        progress(1.0, desc="Reporte generado con éxito!")
...
def create_interface():
    ...
    with gr.Blocks(
        title="EDA Pro - Análisis Exploratorio", 
 ...
 #_________________________________________________________________________________________
            ...
            with gr.TabItem("Gráficos"):
                ...
                def update_plot(plot_type, x_col, y_col):
                    if not x_col:
                        return "Por favor, selecciona una columna X."
                    if plot_type == "Dispersión" and not y_col:
                        return "Por favor, selecciona una columna Y para el gráfico de dispersión."
            ...
            with gr.TabItem(" Reportes"):
                ...
                with gr.Row():
                    ...
                    eda_r_file = gr.File(
                        ...
                        interactive=False,
                        visible=True  # Mantenerlo visible pero no interactivo porque no se alcanzo a terminar
                    )
        ...
        <div style="text-align: center; margin-top: 3rem; padding: 2rem; background-color: #f8f9fa; border-radius: 10px;">
            <p style="color: #666; font-size: 0.9rem;">
                 EDA  v2.0 - Desarrollado para análisis exploratorio <br>
                  Comienza cargando tu dataset y selecciona los análisis 
            </p>
        </div>
        """)
