...
class EDAManager:
    ...
    def generate_ydata_report(self) -> str:
        ...
        try:
            # Generar reporte con configuración mínima sin personalizaciones
            ...
            report_path = os.path.join(temp_dir, f"report_eda_{int(time.time())}.html")
            report.to_file(report_path)
            
            return report_path
            
        except Exception as e:
            logging.error(f"Error al generar reporte ydata: {str(e)}")
   ...
   # __________________________________________________________________________________________________
    def create_custom_plot(self, plot_type: str, x_col: str = None, y_col: str = None) -> Optional[str]:
        ...
        try:
            fig, ax = plt.subplots(figsize=(12, 8))  # Crear figura explícitamente
            
            if plot_type == "Histograma":
                self._plot_histogram(ax, x_col)
            elif plot_type == "Gráfico de barras":
                self._plot_bar_chart(ax, x_col)
            elif plot_type == "Dispersión":
                if y_col is None or y_col == "":
                    return None
                self._plot_scatter(ax, x_col, y_col)
            else:
                return None
                
            ax.grid(True, alpha=0.3)
            fig.tight_layout()
            return self._save_plot(fig)  # Pasar figura explícitamente
            
        except Exception as e:
            logging.error(f"Error al crear gráfico: {str(e)}", exc_info=True)
            return None

  #  ___________________________________________________________________________________________________
  #ESTA ONDA ESTABA DANDO PROBLEMAS 
    def _plot_histogram(self, ax, col_name: str):
        ...
        # Usar el ax proporcionado en lugar de plt
        ax.hist(data, bins=min(30, len(data)), alpha=0.7, edgecolor='black', 
                color='skyblue', density=True)
        ax.set_title(f'Histograma de {col_name}', fontsize=14, fontweight='bold')
        ax.set_xlabel(col_name)
        ax.set_ylabel('Densidad')
    ...
    def _plot_bar_chart(self, ax, col_name: str):
        ...
        ax.set_title(f'Gráfico de Barras - {col_name}', fontsize=14, fontweight='bold')
    ...
    def _plot_scatter(self, ax, x_col: str, y_col: str):
        ...
        ax.scatter(valid_data[x_col], valid_data[y_col], alpha=0.6, s=30, 
                edgecolors='black', linewidth=0.5)
        ax.set_xlabel(x_col)
        ax.set_ylabel(y_col)
        ax.set_title(f'Dispersión: {x_col} vs {y_col}', fontsize=14, fontweight='bold')
